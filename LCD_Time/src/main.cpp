 ...
 * 
 * Backlight:
 * Using BC327 PNP transistor
 * Base - via 10k resistor to PWM pin 6 (of Arduino)
 * Collector - to pin 16 of LCD
 * Emitter - to GND
 * Need pin 6 LOW to turn ON backlight (and pin 6 HIGH to turn OFF backlight) as PNP
 * */
LiquidCrystal lcd(12, 11, 5, 4, 3, 2);
const int backlightPin = 6;

/* LDR to detect when to turn on/off backlight
    Connected to analog pin 5 (A5) */
const int ldrPin = 5;
int light_value;
...
void adjust_backlight() {
  ...
  // If dark - turn OFF backlight
  if (light_value > 700) {
    digitalWrite(backlightPin, HIGH);
  }
  // If bright - turn ON backlight
  else {
...
void setup() {
  ...
  // Built in LED of arduino as output
  pinMode(ledPin, OUTPUT);

  // Backlight control pin as output
